...
void handleButtons(AppState* state) {
  ...
  if (upState && !upPressed) { upPressed = true; handleUp(state); } 
  else if (!upState) { upPressed = false; }
  
  if (downState && !downPressed) { downPressed = true; handleDown(state); }
  else if (!downState) { downPressed = false; }

  if (leftState && !leftPressed) { leftPressed = true; handleLeft(state); }
  else if (!leftState) { leftPressed = false; }
  
  if (rightState && !rightPressed) { rightPressed = true; handleRight(state); }
  else if (!rightState) { rightPressed = false; }

  if (centerState && !centerPressed) { centerPressed = true; handleCenter(state); }
  else if (!centerState) { centerPressed = false; }
}
...
void handleUp(AppState* state) {
  switch (state->currentState) {
    ...
    case WIFI_MANUAL_SETUP:
      ...
      if(state->currentState == SUBMENU_1) drawSubmenu1(state);
      if(state->currentState == WIFI_CONFIG) drawWiFiConfig(state);
      if(state->currentState == WIFI_MANUAL_SETUP) drawWiFiManualSetup(state);
      break;
    case WIFI_SCAN:
  ...
  switch (state->currentState) {
    ...
    case KEYBOARD_INPUT:
      if (state->keyboardRow < 4) {
        state->keyboardCol = (state->keyboardCol - 1 + 10) % 10;
      } else {
        state->keyboardCol = (state->keyboardCol - 1 + 4) % 4;
      }
      drawKeyboard(state);
      break;
  }
...
void handleRight(AppState* state) {
    switch(state->currentState) {
        case KEYBOARD_INPUT:
            if (state->keyboardRow < 4) {
                state->keyboardCol = (state->keyboardCol + 1) % 10;
            } else {
                state->keyboardCol = (state->keyboardCol + 1) % 4;
            }
            drawKeyboard(state);
            break;
    }
...
void handleCenter(AppState* state) {
  switch (state->currentState) {
    case MAIN_MENU:
      if (state->selectedMenuItem == 0) { state->currentState = SUBMENU_1; drawSubmenu1(state); }
    ...
    case SUBMENU_1:
      ...
      break;
    case WIFI_CONFIG:
      if (state->selectedWiFiItem == 0) { // Scan
        state->currentState = WIFI_SCAN;
        scanWiFiNetworks(state);
        drawWiFiScan(state);
      } else if (state->selectedWiFiItem == 1) { // Manual
        state->currentState = WIFI_MANUAL_SETUP;
        state->wifiSSID = "";
        state->wifiPassword = "";
        drawWiFiManualSetup(state);
      } else if (state->selectedWiFiItem == 2) { // Disconnect
        WiFi.disconnect();
        drawWiFiConfig(state);
      } else if (state->selectedWiFiItem == 3) { // Back
        state->currentState = SUBMENU_1;
        drawSubmenu1(state);
      }
      break;
    ...
    case WIFI_MANUAL_SETUP:
      if (state->selectedWiFiItem == 0) {
          state->currentState = KEYBOARD_INPUT; state->isEnteringSSID = true; state->isEnteringPassword = false; state->inputText = state->wifiSSID; drawKeyboard(state);
      } else if (state->selectedWiFiItem == 1) {
          state->currentState = KEYBOARD_INPUT; state->isEnteringPassword = true; state->isEnteringSSID = false; state->inputText = state->wifiPassword; drawKeyboard(state);
  ...
  if (state->keyboardRow < 4) {
    ...
    if (state->keyboardCol == 0) {
      ...
      if (state->isEnteringPassword) {
        ...
        if(state->currentState == WIFI_MANUAL_SETUP) { // From manual setup
             state->currentState = WIFI_MANUAL_SETUP;
             drawWiFiManualSetup(state);
        } else { // From wifi scan
            state->currentState = WIFI_CONNECT;
